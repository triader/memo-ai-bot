...
class WordsService {
    ...
    async getWordsCountInLevel(userId, categoryId, level) {
        const { data, error } = await this.supabase
            .from('words')
    ...
    async getNextAvailableLevel(userId, categoryId, wordsPerLevel) {
        if (!wordsPerLevel)
            return 1;
        const { data, error } = await this.supabase
            .from('words')
    ...
    async getTotalWordsCount(userId, categoryId) {
        const { data, error } = await this.supabase
            .from('words')
            .select('id', { count: 'exact' })
            .eq('user_id', userId)
    ...
    async getWordsByLevel(userId, categoryId, level) {
    ...
    async getCurrentAndMaxLevel(userId, categoryId) {
        const { data, error } = await this.supabase
            .from('words')
            .select('level')
            .eq('user_id', userId)
    ...
    async addWord(userId, categoryId, word, translation) {
        try {
            ...
            if (wordsPerLevel) {
                ...
                const wordsInCurrentLevel = await this.getWordsCountInLevel(userId, categoryId, currentMaxLevel || 1);
    ...
    async deleteWord(userId, wordId, categoryId) {
        try {
    ...
    async getWordsForDeletion(userId, categoryId) {
        const { data, error } = await this.supabase
            .from('words')
            .select('*')
            .eq('user_id', userId)
    ...
    async getWordsWithProgress(userId, categoryId) {
        const { data, error } = await this.supabase
            .from('words')
            .select('*')
            .eq('user_id', userId)
    ...
    async reorganizeWordsIntoLevels(userId, categoryId, wordsPerLevel) {
        try {
