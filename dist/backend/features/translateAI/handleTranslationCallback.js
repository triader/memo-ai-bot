...
async function addWordCallback(bot, callbackQuery) {
    const userId = callbackQuery.from.id;
    const chatId = callbackQuery.message.chat.id;
    const messageId = callbackQuery.message.message_id;
    try {
        const translationKey = callbackQuery.data.replace('add_trans_', '');
        const translationData = translateAIHandler_1.translationStore.get(translationKey);
        if (!translationData) {
...
async function moreExamplesCallback(bot, callbackQuery) {
    const chatId = callbackQuery.message?.chat?.id;
    try {
        ...
        const completion = await config_1.openai.chat.completions.create({
            messages: [
                {
                    role: 'system',
                    content: 'You are a helpful language learning assistant. Provide 5 example sentences using the given word.\n' +
...
async function followUpCallback(bot, callbackQuery) {
    const chatId = callbackQuery.message.chat.id;
    try {
        const followUpDataKey = callbackQuery.data.replace('translate_followup_', '');
        const followUpData = translateAIHandler_1.translationStore.get(followUpDataKey);
        const { translation } = followUpData;
        const sentMessage = await bot.sendMessage(chatId, `What would you like to know?`, {
            reply_markup: { keyboard: [[{ text: constants_1.BUTTONS.CANCEL }]], resize_keyboard: true }
        });
        translateAIHandler_1.translationStore.set(`followup_${chatId}`, {
            word: translation,
...
//# sourceMappingURL=handleTranslationCallback.js.map